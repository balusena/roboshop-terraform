...
module "alb" {
  ...
  vpc_id = lookup(lookup(module.vpc, "main", null), "vpc_id", null)
  sg_subnet_cidr = each.value[name] == "public" ? ["0.0.0.0/0"] : concat(lookup(lookup(lookup(lookup(var.vpc, "main", null),
    "subnets", null), "web", null), "cidr_block", null), lookup(lookup(lookup(lookup(var.vpc, "main", null), "subnets", null),
    "app", null), "cidr_block", null))
